...
// Reset trial: clear secure storage and create a fresh 14-day trial
#[tauri::command]
pub async fn reset_trial(app: AppHandle) -> Result<ActivationResponse, String> {
    // Remove any existing stored license and model selection
    let _ = secure_storage_remove(
        app.clone(),
        vec![
            "Scribe_license_key".to_string(),
            "Scribe_instance_id".to_string(),
            "selected_Scribe_model".to_string(),
        ],
    )
    .await;

    // Create a new trial license
    create_trial_license(app).await
}