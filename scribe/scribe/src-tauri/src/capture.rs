...
pub async fn start_screen_capture(app: tauri::AppHandle) -> Result<(), String> {
    // Get primary monitor (xcap) for capturing the screen image (physical pixels)
    ...
    *state.captured_image.lock().unwrap() = Some(captured_image);
    ...
    let (logical_w, logical_h, logical_x, logical_y) = {
        ...
        let mut lx = primary_monitor.x() as f64;
        let mut ly = primary_monitor.y() as f64;

        if let Ok(Some(m)) = app.primary_monitor() {
            let sf = m.scale_factor();
            let size = m.size(); // physical size
            let pos = m.position(); // physical position
            if sf > 0.0 {
                lw = (size.width as f64) / sf;
                lh = (size.height as f64) / sf;
                lx = (pos.x as f64) / sf;
                ly = (pos.y as f64) / sf;
            }
        }
