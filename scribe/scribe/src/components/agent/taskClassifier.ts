/**
 * Heuristic: classify user input as "task" (use Pi agent: plan, steps, tools)
 * vs "conversation" (use Ollama with chat history).
 * Can be replaced later with an LLM-based classifier.
 */
...
const TASK_PATTERNS = [
  ...
  /^[\w\s]{0,80}\s*[.!]$/, // Short imperative sentence ending with . or !
];
...
/**
 * Returns true if the text looks like a task (action/goal for Pi agent),
 * false if it looks like conversation (use Ollama with history).
 */
